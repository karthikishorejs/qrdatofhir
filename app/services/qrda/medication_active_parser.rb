...
# Extracts active medications (templateId ...24.3.41)
class MedicationActiveParser
  ...
  def self.extract(doc, ns)
    MedicationExtraction.all_substance_administration_nodes(doc, ns).filter_map do |node|
      ...
      next unless template_ids.include?(MedicationExtraction::TPL_MED_ACTIVE)

      # Exclude discharge list meds if they happen to also include active template ids
